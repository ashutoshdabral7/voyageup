...
class Device_communication_model extends CI_Model {
    ...
    public function login_user($userdetails){
        ...
        if(!empty($userdetails)){
            ...
            @$result_set->auth_key = '';
            if($numrows > 0){
				if($result_set->verified == 1 || $result_set->LoginType == 'F'){
    ...
    function invite($auth_key,$post_array){
        ...
        if(!empty($post_array)){
		   ...
		   if(isset($post_array['notification_receiver'])){
				...
				$userData = $this->common_model->selectAll('User', array('UserId' => $post_array['notification_receiver']));
		   
				if(!empty($userData) && $userData[0]->device_id != ""){
					...
					$msg['short_descr'] = $post_array['NotificationMessage'];
					...
					push_message($msg, $userData[0]->device_id);			
				}
		   }
